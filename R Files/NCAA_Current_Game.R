...
for (team in allTeams) {
  ...
  gameDate = DateCheck
    ...
    print(gameDate)
    game = subset(GameTotals, GameTotals$Tm == team & as.Date(GameTotals$GameDate) < as.Date(gameDate)) # all games before the date
    currentGame = subset(OffensiveStatsNCAAToday, as.Date(OffensiveStatsNCAAToday$Date) == as.Date(gameDate)  & 
                           OffensiveStatsNCAAToday$Tm == team)
    if (nrow(game) == 0 | nrow(currentGame) == 0){
      next
    }
    ...
    temp$GameDate = gameDate
    temp$Opp = currentGame$Opp[1]
    temp$Points = mean(game$Points)
    temp$HW = currentGame$Home[1]
    temp$Actual = 0
    DataNCAAPoints = rbind(temp, DataNCAAPoints)
