DROP VIEW IF EXISTS [dbo].[NBA_Shots_View_All]
GO
...
AS
	...
	SELECT
		PVT.GameDate, PVT.PlayerName, PVT.FreeThrows, ISNULL([Right 3], 0) 'R3', 
	...
	FROM
	 (
		SELECT 
			PV.GameDate, PV.PlayerName, PV.FreeThrows, PV.Shot, [Right 3], [Left 3], [Center 3], [In the Paint], [Right Corner 3], [Left Corner 3], [Back 3], [Left 2], [Right 2], [Center 2]
		FROM

			(		
			SELECT 
				A.GameDate, A.PlayerName, CAST(count(A.Shot) as int) Totals, CAST(A.FreeThrows as int) FreeThrows, A.Zones, A.Shot
			FROM
			(
				Select 
					Shot.*,
			...
			Group BY 
				A.PlayerName, A.Shot, A.Zones, A.FreeThrows, A.GameDate
			) V
			PIVOT(
				SUM(V.Totals)
	...
	PVT
	--ORDER BY PVT.GameDate ASC, PVT.PlayerName ASC
GO