...
-- exec usp_GetTopScorerShotChart @team_ = 'TOR'
DROP PROCEDURE IF EXISTS usp_GetTopScorerShotChart
GO

CREATE PROCEDURE usp_GetTopScorerShotChart
(
...
BEGIN
	...
	WITH topScorers as (
		Select 
			Tm, [date], PlayerName, Count(*) totals
		FROM 
			NBA_PlayerLog
		WHERE
			Opp = @team_ AND PTS > 18
		GROUP BY 
			Tm ,  [date], PlayerName	
		)
		...
		FROM
			...
			INNER JOIN topScorers TP ON TP.[Date] =  Shot.GameDate and TP.PlayerName = NBA.PlayerName
		) A
		Group BY 
			A.PlayerName,A.PlayerPosition, A.Shot, A.Zones, A.FreeThrows, A.GameDate, A.PlayerTeam, A.Opp
		ORDER BY
			GameDate DESC, A.PlayerName asc, Shot ASC, totals DESC
